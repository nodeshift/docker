...
1. Generate a Linux version of your `node_modules` dependencies locally, by generating them inside the node:12 docker image:
  ```sh
  docker run -i -v "$PWD"/package.json:/tmp/package.json -v "$PWD"/node_modules_linux:/tmp/node_modules -w /tmp -t node:12 npm install
  ```
...
* Retrieves your dependencies and compiles any native add-ons using the node:12 image
* Copies your dependencies into the node:12-slim image for reduced size
* Runs your application under the `node` user 
